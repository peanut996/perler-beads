...
import { detectEnvironment } from './downloadUtils';
...
export async function downloadImage({
  ...
  const processDownload = () => {
    ...
    ctx.fillText('浦子哥爱画图', titleStartX, mainTitleY);
    ...
    if (includeStats && colorCounts) {
      ...
      const statsWatermarkText = '图纸来源：@浦子哥爱画图';
    ...
    try {
      downloadCanvas.toBlob(async (blob) => {
        ...
        // WebView: Show guidance modal
        if (env.isWebView && options.onWebViewDetected) {
          options.onWebViewDetected(env.webViewApp);
          // 不继续执行下载，因为 Modal 将指导用户
          return;
        }

        // iOS Safari: Use Web Share API
        if (env.isIOSSafari && navigator.share && navigator.canShare) {
          ...
          if (navigator.canShare({ files: [file] })) {
            try {
              ...
              console.log('Web Share failed, falling back to blob download', shareErr);
            }
          }
        }
        ...
        console.log("Grid image download initiated via Blob.");
